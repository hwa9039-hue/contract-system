...
router = APIRouter(prefix="/api/excluded-projects", tags=["excluded-projects"])
...
def prepare_insert_values(row: ExcludedProjectCreate) -> dict:
    values = excluded_project_to_db_values(row)
    timestamp = now_text()
    values["id"] = str(uuid4())
...
def insert_excluded_project_row(cursor, row: ExcludedProjectCreate) -> dict:
    ...
    cursor.execute(
        f"""
        insert into excluded_projects_rows ({", ".join(quoted_columns)})
        values ({", ".join(placeholders)})
        returning {RETURNING_COLUMNS}
        """,
        values,
    )
    return row_to_excluded_project(cursor.fetchone())


@router.get("", response_model=list[ExcludedProjectOut])
def list_excluded_project_rows():
    with get_connection() as connection:
        with connection.cursor() as cursor:
            ...
            return [row_to_excluded_project(row) for row in cursor.fetchall()]


@router.post("", response_model=ExcludedProjectOut, status_code=status.HTTP_201_CREATED)
def create_excluded_project_row(row: ExcludedProjectCreate):
    with get_connection() as connection:
        with connection.cursor() as cursor:
            created = insert_excluded_project_row(cursor, row)
        connection.commit()
...
def update_excluded_project_row(row_id: str, patch: ExcludedProjectPatch):
    ...
    return row_to_excluded_project(updated)


@router.delete("/{row_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_excluded_project_row(row_id: str):
    with get_connection() as connection:
        with connection.cursor() as cursor:
            cursor.execute("delete from excluded_projects_rows where id::text = %s", (row_id,))
...
def bulk_delete_excluded_project_rows(payload: ExcludedProjectBulkDelete):
    ...
    with get_connection() as connection:
        with connection.cursor() as cursor:
            cursor.execute("delete from excluded_projects_rows where id::text = any(%s)", (ids,))
            deleted_count = cursor.rowcount
        connection.commit()
...
@router.delete("")
def delete_all_excluded_project_rows():
    with get_connection() as connection:
        with connection.cursor() as cursor:
            cursor.execute("delete from excluded_projects_rows")
            deleted_count = cursor.rowcount
        connection.commit()
