...
from app.routers.bit_history import router as bit_history_router
from app.routers.payment_reports import router as payment_reports_router
...
# 환경변수에 도메인을 빠뜨려도 회사 프론트(pages.dev·smart-di.com)가 허용되도록 정규식으로 보조합니다.
ALLOW_ORIGIN_REGEX = (
...
def on_startup():
    ...
    logger.info("Payment reports API: GET/POST /api/payment-reports (app.include_router payment_reports_router)")
...
app.include_router(bit_history_router)
